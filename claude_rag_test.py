...
Flow: User Query → Claude (generates search query) → Indian Kanoon API
      → Claude (synthesises answer) → Claude (generates follow-up recommendations)
      → User-friendly response

User sees  : plain-language answer + follow-up question recommendations
...
# ── Step 5: Claude → recommended follow-up questions ─────────────────────────
RECOMMENDATIONS_PROMPT = """
You are a legal assistant helping Indian users navigate their legal situation.

Based on the original question and the answer just provided, generate exactly 3 follow-up questions the user is LIKELY to ask next.

STRICT RULES:
- Output a JSON array of exactly 3 strings. Nothing else. No markdown, no preamble.
- Each question must be SHORT (under 15 words), plain language, and feel natural to ask.
- Questions must be ANSWERABLE using Indian Kanoon case law or Indian statutes — no questions needing personal legal advice, document review, or facts only the user knows.
- Questions must be SPECIFIC to the legal topic in this conversation — not generic ("What is my next step?").
- Cover different angles: e.g. one on process/procedure, one on rights/remedies, one on time limits or exceptions.
- No question should repeat or paraphrase the original question.

Output format example:
["Question one here?", "Question two here?", "Question three here?"]
"""

def generate_recommendations(user_question: str, answer: str) -> list[str]:
    """
    Ask Claude to generate 3 context-aware follow-up questions.
    Returns a list of question strings. Falls back to [] on any error.
    """
    t0 = time.time()
    try:
        resp = claude.messages.create(
        ...
        elapsed = time.time() - t0
        log.info(f"RECOMMENDATIONS | count={len(questions)}  time={elapsed:.2f}s")
        for i, q in enumerate(questions, 1):
            log.debug(f"  REC {i} | {q!r}")
...
def rag_query(user_question: str) -> str:
    ...
    # 5. Fetch top 2 relevant cases from IK API
    ...
    recommendations = generate_recommendations(user_question, answer)
